export enum ChainId {
  Mainnet = 1,
  Ropsten = 3,
  Rinkeby = 4,
  Goerli = 5,
  Kovan = 42,
  BSC = 56,
  BSCTestnet = 97,
  xDai = 100,
  Polygon = 137,
  Theta = 361,
  ThetaTestnet = 365,
  Moonriver = 1285,
  Moonbase = 1287,
  Mumbai = 80001,
  Harmony = 1666600000,
  Palm = 11297108109,
  Localhost = 1337,
  LocalAvalanche = 43112,
  Moondev = 1281,
  Hardhat = 31337,
  Fantom = 250,
  FantomTestnet = 4002,
  AvalancheFuji = 43113,
  AvalancheMainnet = 43114,
  TomoMainnet = 88,
  TomoTestnet = 89,
  SmartBchTestnet = 10001,
  SmartBchMainnet = 10000,
}

export const CHAIN_NAMES = {
  [ChainId.Mainnet]: 'Mainnet',
  [ChainId.Ropsten]: 'Ropsten',
  [ChainId.Kovan]: 'Kovan',
  [ChainId.Rinkeby]: 'Rinkeby',
  [ChainId.Goerli]: 'Goerli',
  [ChainId.BSC]: 'BSC',
  [ChainId.BSCTestnet]: 'BSCTestnet',
  [ChainId.xDai]: 'xDai',
  [ChainId.Polygon]: 'Polygon',
  [ChainId.Theta]: 'Theta',
  [ChainId.ThetaTestnet]: 'ThetaTestnet',
  [ChainId.Moonriver]: 'Moonriver',
  [ChainId.Moonbase]: 'Moonbase',
  [ChainId.Mumbai]: 'Mumbai',
  [ChainId.Harmony]: 'Harmony',
  [ChainId.Palm]: 'Palm',
  [ChainId.Localhost]: 'Localhost',
  [ChainId.LocalAvalanche]: 'LocalAvalanche',
  [ChainId.Moondev]: 'Moondev',
  [ChainId.Hardhat]: 'Hardhat',
  [ChainId.Fantom]: 'Fantom',
  [ChainId.FantomTestnet]: 'FantomTestnet',
  [ChainId.AvalancheFuji]: 'AvalancheFuji',
  [ChainId.AvalancheMainnet]: 'AvalancheMainnet',
  [ChainId.TomoTestnet]: 'TomoTestnet',
  [ChainId.TomoMainnet]: 'TomoMainnet',
  [ChainId.SmartBchTestnet]: 'SmartBchTestnet',
  [ChainId.SmartBchMainnet]: 'SmartBchMainnet',
}

export const COIN_NAMES = {
  [ChainId.Mainnet]: 'ETH',
  [ChainId.Ropsten]: 'ETH',
  [ChainId.Kovan]: 'ETH',
  [ChainId.Rinkeby]: 'ETH',
  [ChainId.Goerli]: 'ETH',
  [ChainId.BSC]: 'BNB',
  [ChainId.BSCTestnet]: 'BNB',
  [ChainId.xDai]: 'XDAI',
  [ChainId.Polygon]: 'MATIC',
  [ChainId.Theta]: 'THETA',
  [ChainId.ThetaTestnet]: 'THETA',
  [ChainId.Hardhat]: 'ETH',
  [ChainId.Localhost]: 'ETH',
  [ChainId.LocalAvalanche]: 'AVAX',
  [ChainId.Moondev]: 'DEV',
  [ChainId.Moonriver]: 'MOVR',
  [ChainId.Moonbase]: 'DEV',
  [ChainId.Mumbai]: '?',
  [ChainId.Palm]: '?',
  [ChainId.Harmony]: 'ONE',
  [ChainId.Fantom]: 'FTM',
  [ChainId.FantomTestnet]: 'FTM',
  [ChainId.AvalancheFuji]: 'AVAX',
  [ChainId.AvalancheMainnet]: 'AVAX',
  [ChainId.TomoTestnet]: 'TOMO',
  [ChainId.TomoMainnet]: 'TOMO',
  [ChainId.SmartBchTestnet]: 'BCH',
  [ChainId.SmartBchMainnet]: 'BCH',
}

export const MULTICALL_ADDRESSES = {
  [ChainId.Mainnet]: '0xeefba1e63905ef1d7acba5a8513c70307c1ce441',
  [ChainId.Ropsten]: '0x53c43764255c17bd724f74c4ef150724ac50a3ed',
  [ChainId.Rinkeby]: '0x42ad527de7d4e9d9d011ac45b31d8551f8fe9821',
  [ChainId.Goerli]: '0x77dca2c955b15e9de4dbbcf1246b4b85b651e50e',
  [ChainId.Kovan]: '0x2cc8688c5f75e365aaeeb4ea8d6a480405a48d2a',
  [ChainId.BSC]: '0x41263cba59eb80dc200f3e2544eda4ed6a90e76c',
  [ChainId.BSCTestnet]: '0xae11C5B5f29A6a25e955F0CB8ddCc416f522AF5C',
  [ChainId.xDai]: '0xb5b692a88bdfc81ca69dcb1d924f59f0413a602a',
  [ChainId.Polygon]: '0x11ce4B23bD875D7F5C6a31084f55fDe1e9A87507',
  [ChainId.Theta]: '0xe2ec58a54f3ab2714eddbae87533793011f1e14e',
  [ChainId.ThetaTestnet]: '0xf822bf2e728e264c58d7618022addd9cbc780350',
  [ChainId.Moonriver]: '0xa9177F8d98DAaB74C24715Ba0A81b73654710523',
  [ChainId.Moonbase]: '0xEf41Aa0Df9CEeB5bcD30a8e183dB40f73cd70e54',
  [ChainId.Mumbai]: '0x08411ADd0b5AA8ee47563b146743C13b3556c9Cc',
  [ChainId.Harmony]: '0xFE4980f62D708c2A84D3929859Ea226340759320',
  [ChainId.Palm]: '0x99a73dfE34578348fb81BD078201C0BA84E9c840',
  [ChainId.Fantom]: '0xdc85396592f0F466224390771C861EE3957a3ff4',
  [ChainId.FantomTestnet]: '0xbC44a1533F9d733404eDe4c4cEee24de0FB397E1',
  [ChainId.AvalancheFuji]: '0x657c6F6B6Ba052238Dc11940087b7Ec36a3a33D6',
  [ChainId.SmartBchTestnet]: '0x727b5C444e882198774713d8D769C29f54cD3d83',
  [ChainId.TomoTestnet]: '0xd46C8D45e2441580a6e45d49DA3170fc4533bEc6',
}

export const TEST_CHAINS = [
  ChainId.Ropsten,
  ChainId.Kovan,
  ChainId.Rinkeby,
  ChainId.Goerli,
  ChainId.Mumbai,
  ChainId.ThetaTestnet,
  ChainId.Localhost,
  ChainId.Hardhat,
  ChainId.FantomTestnet,
  ChainId.AvalancheFuji,
  ChainId.TomoTestnet,
  ChainId.SmartBchTestnet,
]

export const LOCAL_CHAINS = [
  ChainId.Localhost,
  ChainId.LocalAvalanche,
  ChainId.Moondev,
  ChainId.Hardhat
]
